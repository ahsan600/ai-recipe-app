import { AuthErrorResponse, AuthResponse } from '@/types/AuthTypes'
import {
	AuthError,
...
import { AuthStateCallback, SignInWithEmailArgs, SignUpWithEmailArgs } from './types'

export function onAuthStateChanged(callback: AuthStateCallback): Unsubscribe {
	return fbOnAuthStateChanged(auth, callback)
}

export async function signInWithEmail(args: SignInWithEmailArgs): Promise<AuthResponse<User>> {
	try {
...
export async function signUpWithEmail(args: SignUpWithEmailArgs): Promise<AuthResponse<User>> {
	try {
...
export async function sendVerificationEmail(user: User): Promise<AuthResponse<null>> {
	try {
		if (user) {
...
export function handleAuthError(error: AuthError): AuthErrorResponse {
	const errorMessages: Record<string, string> = {
		...
		'auth/multi-factor-auth-required': 'Multi-factor authentication is required for this account.',
		...
		'auth/requires-recent-login': 'For security, please sign in again to perform this action.',
